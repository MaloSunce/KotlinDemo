...
object FavoritePlants {
    ...
    fun initFavorites(context: Context): Error? {
        ...
        return if (file.exists()) {
            try {
                ...
                // TODO: adding test plants if list is empty, test code
                if (plants.size < 1) {
    ...
    // where favorite plants are permanently stored
    ...
    suspend fun removeFavorite(plant: FavoritePlant): Error? {
        ...
        try {
            ...
            Log.e("PLANT_DEBUG", "Failed to remove plant with id ${plant.id} from database: ${Log.getStackTraceString(e)}")
            return Error("Failed to remove plant from favorites. Please try again.")
        }
    ...
    fun updatePersistentStorage(): Error? {
        return try {
...
data class FavoritePlant(
    ...
    val imageURL: Int = -1, // Uses default image if -1
